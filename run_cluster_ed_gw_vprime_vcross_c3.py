#!/usr/bin/env python3
"""Run a C3-constrained V' + Vx cluster-ED+GW normal-state background.

The ordinary six-site cluster correction can select one primitive-cell
orientation at strong coupling.  This launcher keeps the *physical lattice*
self-energy on the exact Ruby C3-invariant manifold using the momentum-dependent
rotation/gauge transformation implemented in ``rubycgw.c3_constraint``.

The internal impurity is not locally permuted.  Its correction is embedded and
then C3 projected on the full lattice.  The resulting checkpoint is intended as
a symmetric normal-state reference for the ordinary, unconstrained JF response
scanner.
"""
...
def _args():
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="seed the constrained branch from an existing embedded checkpoint",
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("results/vprime_vcross_cluster_bg_c3"),
    )
    return p.parse_args()
...
def main():
    args = _args()
    if args.Lx != args.Ly:
        raise ValueError("C3-constrained background requires Lx=Ly")
    ...
    print(
        "=== Ruby C3-constrained V-prime + V-cross cluster ED+GW ===\n"
        f"L={args.Lx}x{args.Ly}, V={args.V:g}, V'={args.Vprime:g}, "
        f"Vx={args.Vcross:g}, filling={args.filling:g}, T={args.T:g}\n"
        "constraint: exact physical lattice C3 projection of Sigma_emb; "
        "JF should be run unconstrained afterwards",
        flush=True,
    )
    ...
    outfile = args.out / (
        f"cluster_ed_gw_vprime_vcross_c3_L{args.Lx}x{args.Ly}_V{_tag(args.V)}_"
    ...
    np.savez_compressed(
        ...
        c3_constrained=np.asarray(True),
        c3_constraint_kind=np.asarray("physical_lattice_sigma_embedding_projection"),
        c3_response_policy=np.asarray("background_constrained_response_unconstrained"),
        c3_h0_residual=float(result.c3_h0_residual),
        c3_final_residual=float(result.c3_final_residual),
        c3_final_density_raw_spread=float(result.c3_final_density_raw_spread),
        c3_raw_residual_history=np.asarray(result.c3_raw_residual_history),
        c3_removed_norm_history=np.asarray(result.c3_removed_norm_history),
        c3_density_raw_spread_history=np.asarray(result.c3_density_raw_spread_history),
        ...
        background_kind=np.asarray(
            "c3_constrained_parameter_continuation" if restart is not None
            else "c3_constrained_standalone_scgw"
        ),
        Lx=int(args.Lx), Ly=int(args.Ly),
    ...
    print(
        ...
        f"C3(Sigma)={result.c3_final_residual:.3e}, "
        f"density-spread={result.c3_final_density_raw_spread:.3e}",
        flush=True,
    )
