"""Lightweight adaptive solver for the 18-site period-three Ruby GW problem.

The physical GW equations are unchanged.  This module is intentionally much
simpler than the previous trust-region implementation:

* a cold start uses the analytic uniform Hartree field implied by the requested
  filling, rather than Sigma_H=0;
* Hartree and dynamic-GW blocks are mixed with different damping factors;
* every outer iteration evaluates the expensive GW map only once;
* the chemical potential uses the cached-tail safeguarded Newton solver;
* Anderson acceleration is used only late, after the raw fixed-point residual
  has entered a small, stable basin;
* screening SVD diagnostics are sampled sparsely during verbose runs and are
  evaluated exactly once again on the returned state.

This keeps the numerical path cheap while retaining the same fixed point
Sigma = F[Sigma].
"""
...
class AndersonOptions:
    """Numerical controls kept API-compatible with the driver.

    The old command-line names are retained so existing scripts continue to
    run.  Only a subset is active in the lightweight solver.  The important
    new defaults are ``hartree_beta=0.30`` and ``gw_beta=0.15``.
    """
...
def _late_anderson_step(
    ...
    """One Type-II Anderson proposal, without an expensive line search."""
    if len(history) < 2:
        return sigma_h, sigma_gw, False

    hscale = max(_rms(sigma_h), scale_floor)
    gwscale = max(_rms(sigma_gw), scale_floor)
    ...
    if step_norm > step_cap * max(linear_norm, 1e-16):
        return sigma_h, sigma_gw, False
...
def _uniform_hartree_seed(
    ...
    """Analytic uniform-density Hartree seed for a cold start.

    For the half-filled Ruby 18-site supercell this evaluates to 2 V I_18.
    """
...
def solve_matrix_gw_anderson(
    ...
    """Solve matrix GW using cheap block mixing and late Anderson acceleration."""
    _validate(anderson)
    ...
    if not cold_start:
        ...
        # A uniform Hartree field shifts all one-particle energies by its mean
        # diagonal value.  Starting mu with the same shift greatly reduces the
        # first fixed-filling search; the Newton solve below still enforces the
        # requested filling exactly.
    ...
    for it in range(1, opts.max_iter + 1):
        ...
        # A bad late-Anderson step costs only this one map evaluation.  Drop its
        # history immediately and continue with block-linear mixing; no rollback
        # or extra trial maps are performed.
        ...
        # Adapt only the dynamic-GW damping.  Hartree is cheap, static and much
        # better conditioned, so it keeps a larger fixed step.
        ...
        if opts.verbose and (it == 1 or it % int(anderson.diagnostic_interval) == 0):
            ...
            # Near a sampled screening pole, cap future GW steps without doing
            # another expensive map or SVD on every iteration.
            if last_smin < 5e-3:
                gw_beta = min(gw_beta, 0.04)
            elif last_smin < 2e-2:
                gw_beta = min(gw_beta, 0.08)
    ...
    # Strict fixed-filling polish and exact residual on the state we return.
